...
/**
 * Implementation of the {@link DomibusConnectorDeliveryClient} interface that handles the reception
 * of messages delivered by the Domibus Connector in push mode.
 */
@Component
@ConditionalOnProperty(
    prefix = ConnectorLinkWSProperties.PREFIX,
    value = ConnectorLinkWSProperties.PUSH_ENABLED_PROPERTY_NAME
)
    ...
    public void receiveDeliveredConfirmationMessageFromConnector(
        ...
        LOGGER.info(
            "#receiveDeliveredConfirmationMessageFromConnector: "
                + "received new confirmation from connector via push."
        );
        try {
        ...
        LOGGER.debug(
            "receiveDeliveredConfirmationMessageFromConnector: "
                + "received confirmation delivered to client backend."
        );
    }
}
