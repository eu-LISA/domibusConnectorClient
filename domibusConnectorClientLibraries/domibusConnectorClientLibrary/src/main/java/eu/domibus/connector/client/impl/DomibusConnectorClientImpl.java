...
/**
 * The DomibusConnectorClientImpl class is an implementation of the DomibusConnectorClient
 * interface. It provides methods for submitting and processing messages in the Domibus Connector.
 */
...
public class DomibusConnectorClientImpl implements DomibusConnectorClient {
    ...
    public void submitNewMessageToConnector(DomibusConnectorMessageType message)
        ...
        if (!domibusConnectorAckType.isResult()) {
            LOGGER.error(
                "The received acknowledgement for message with backend message "
                    + "ID {} is negative! {} ",
                message.getMessageDetails().getBackendMessageId(),
     ...
     * {@inheritDoc}
     *
     * @deprecated - This method is deprecated and should not be used. It will be removed in future
     *      versions.
     */
    @Override
    @Deprecated
    ...
    public Map<String, DomibusConnectorMessageType> requestNewMessagesFromConnector(
        ...
        try {
            ...
            LOGGER.error(
                "Exception occurred requesting list of pending message transport "
                    + "IDs from connector!"
            );
            throw e;
        }
        ...
        if (!CollectionUtils.isEmpty(pendingMessageTransportIds)) {
            ...
            for (var pendingMessageTransportId : pendingMessageTransportIds) {
                ...
                try {
                    ...
                    // The message transport ID will then most likely be contained in the next
                    // call of listPendingMessages.
                    LOGGER.error(
                        "Exception occurred requesting message with transport id {} from "
                            + "connector!", pendingMessageTransportId
                    );
                    continue;
                }
    ...
    public void triggerConfirmationForMessage(DomibusConnectorMessageType confirmationMessage)
        ...
        if (confirmationMessage.getMessageConfirmations() == null
            ...
            throw new DCCMessageDataInvalid(
                "The confirmationMessage must contain one messageConfirmation. "
                    + "This messageConfirmation must contain the confirmationType that should "
                    + "be generated and submitted by the connector!"
            );
        }
        ...
        if (domibusConnectorAckType == null) {
            LOGGER.error(
                "The received acknowledgement for confirmation message with originalEbmsId {} and "
                    + "confirmationType {} is null! ", refToMessageId, confirmationType.name()
            );
            throw new DCCConnectorAcknowledgementException(
        ...
        if (!domibusConnectorAckType.isResult()) {
            LOGGER.error(
                "The received acknowledgement for confirmation message with "
                    ...
                    + "ResultMessage: {}",
                refToMessageId, confirmationType.name(), domibusConnectorAckType.getResultMessage()
            );
            throw new DCCConnectorAcknowledgementException(
                "The received acknowledgement for confirmation message with originalEbmsId "
                    + refToMessageId + " and confirmationType " + confirmationType.name()
